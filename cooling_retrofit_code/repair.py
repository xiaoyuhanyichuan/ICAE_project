...
_DEFAULT_WEIGHT_ABS_TOLERANCE_KG = 1.0e-6
_DEFAULT_WEIGHT_REL_TOLERANCE = 1.0e-9
...
def repair_vector(
    ...
    weight_abs_tolerance_kg: float = _DEFAULT_WEIGHT_ABS_TOLERANCE_KG,
    weight_relative_tolerance: float = _DEFAULT_WEIGHT_REL_TOLERANCE,
    ...
    weight_margins = _validate_weight_margins(schema.zone_ids, floor_weight_margin_by_zone_kg)
    ...
    for zone_idx, zone_id in enumerate(schema.zone_ids):
        ...
        if config in _AC_CONFIGS:
            ac_limit_kw = min(peak, upper[capacity_offsets["cap_ac_new"] + zone_idx])
            _raise_capacity(
                repaired,
                capacity_offsets["cap_ac_new"] + zone_idx,
                min(max(0.0, peak - existing_eff), ac_limit_kw),
                zone_id,
                "cap_ac_new",
                actions,
            )
        ...
        if config in _RDHX_CONFIGS:
            rdhx_limit_kw = min(peak * rdhx_fraction, upper[capacity_offsets["cap_rdhx"] + zone_idx])
            _raise_capacity(
                repaired,
        ...
        if config in _CDU_CONFIGS:
            cdu_limit_kw = min(peak * cold_plate_fraction, upper[capacity_offsets["cap_cdu"] + zone_idx])
            _raise_capacity(
                repaired,
        ...
        _limit_zone_weight(
            ...
            zone_id,
            capacity_offsets,
            weight_margins[zone_id],
            equipment_weights,
            actions,
            weight_abs_tolerance_kg=weight_abs_tolerance_kg,
...
def screen_decision(
    ...
    cold_plate_fraction: float = _DEFAULT_COLD_PLATE_FRACTION,
    rdhx_fraction: float = _DEFAULT_RDHX_FRACTION,
    disabled_hard_checks: set[str] | None = None,
    weight_abs_tolerance_kg: float = _DEFAULT_WEIGHT_ABS_TOLERANCE_KG,
    weight_relative_tolerance: float = _DEFAULT_WEIGHT_REL_TOLERANCE,
) -> ScreeningResult:
    ...
    disabled = {str(item) for item in (disabled_hard_checks or set())}
    ...
    for zone_id in decision.zone_ids:
        ...
        weight_tolerance = _weight_tolerance_kg(
            weight_margins[zone_id],
            weight_abs_tolerance_kg,
            weight_relative_tolerance,
        )
        if "weight" not in disabled and zone_weight > weight_margins[zone_id] + weight_tolerance:
            excess = zone_weight - weight_margins[zone_id]
            violation += excess
            reasons.append(f"{zone_id}:weight_margin_exceeded:{excess}")

        peak = peak_loads[zone_id]
        existing_air_capacity = old_air_capacity[zone_id] if decision.s_z[zone_id] in _EXISTING_AC_CONFIGS else 0.0
        supplied = existing_air_capacity + _zone_effective_peak_cooling_capacity(
            decision,
            zone_id,
            peak,
            cold_plate_fraction,
        ...
        if "zone_peak" not in disabled and supplied < peak:
    ...
    if "chiller_peak" not in disabled and chiller_capacity < chiller_load_bound:
    ...
    if "tower_peak" not in disabled and tower_capacity < tower_heat_bound:
...
def _limit_capacity(
    ...
    limit = max(0.0, float(limit_kw))
    if repaired[offset] > limit + tolerance:
        repaired[offset] = limit
        actions.append(f"{zone_id}:limit_{_action_capacity_name(capacity_name)}_to_effective_peak")
...
def _limit_zone_weight(
    ...
    if total_weight <= weight_margin_kg + tolerance or total_weight <= 0.0:
        return

    target_margin = max(0.0, weight_margin_kg)
    scale = target_margin / total_weight
    for offset, _ in weighted_offsets:
        repaired[offset] *= scale
    actions.append(f"{zone_id}:scale_capacity_to_weight_margin")


def _weight_tolerance_kg(
    weight_margin_kg: float,
    abs_tolerance_kg: float = _DEFAULT_WEIGHT_ABS_TOLERANCE_KG,
    relative_tolerance: float = _DEFAULT_WEIGHT_REL_TOLERANCE,
) -> float:
    abs_tol = max(0.0, float(abs_tolerance_kg))
    rel_tol = max(0.0, float(relative_tolerance)) * max(1.0, abs(float(weight_margin_kg)))
...
def _zone_effective_peak_cooling_capacity(
    decision: PlanningDecision,
    zone_id: str,
    peak_kw: float,
    cold_plate_fraction: float = _DEFAULT_COLD_PLATE_FRACTION,
    ...
    rdhx_fraction = min(max(float(rdhx_fraction), 0.0), 1.0)
    ac_new = (
        min(float(decision.cap_ac_new[zone_id]), peak)
    ...
    return ac_new + cp_bound + rdhx_bound
